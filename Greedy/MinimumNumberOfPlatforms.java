...
Solution:
	...
	Another efficient solution is O(n) on geeks page
...
public class MinimumNumberOfPlatforms {
	static int findPlatform(int arr[], int dep[], int n)
    {
		...
		for(int i=1; i<intervals.length; i++) {
			...
			if(intervals[i].start > prevDeparture) {	//Note that if start of next train = end of prev, then we need extra platform as per the question
				minHeap.remove();
			}
